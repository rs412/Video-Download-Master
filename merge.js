...
(function (global) {
  ...
  // mergeBlobs(videoBlob, audioBlob, onStage) → Blob (video/mp4)
  // 纯 remux 两个已就绪的 Blob（后台 offscreen B 站任务用：轨道已在别处下好）
  async function mergeBlobs(videoBlob, audioBlob, onStage) {
    const M = await ready();
    const {
    ...
    await pumpPackets(
      ...
      (p) => report(0.05 + 0.5 * p, "封装视频轨 " + Math.round(p * 100) + "%…")
    );
    report(0.6, "封装音轨…");
    await pumpPackets(
      aTrack, aSource, aConfig,
      (p) => report(0.6 + 0.3 * p, "封装音轨 " + Math.round(p * 100) + "%…")
    );

    report(0.92, "生成 mp4 容器…");
  ...
  // 下载两路直链（googlevideo 等，无 Referer 限制）+ remux
  async function mergeAv(videoUrl, audioUrl, onStage) {
    ...
    return await mergeBlobs(videoBlob, audioBlob, (p, msg) =>
      report(0.72 + 0.28 * p, msg)
    );
  }

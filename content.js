...
(function () {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    // 腾讯：页面主世界逐片下载（腾讯 CDN 对扩展 origin 断流；页面自身能播放 = 页面环境必通）。
    // 复用主世界 __vdm_bili_dash_track_req__ 下载器（流式 + transferable 回传），
    // 本侧不攒分片内存，逐片转 base64 经 runtime 消息流转给 offscreen 累积合并。
    if (msg.type === "TENCENT_PAGE_RELAY") {
      const id = "__tq_relay_" + Date.now() + "_" + Math.floor(Math.random() * 1e6);
      const abToB64 = (buf) => {
        const u = new Uint8Array(buf);
        let s = "";
        const CH = 0x8000;
        for (let i = 0; i < u.length; i += CH) {
          s += String.fromCharCode.apply(null, u.subarray(i, Math.min(i + CH, u.length)));
        }
        return btoa(s);
      };
      let closed = false;
      let watchdog = setTimeout(() => {
        ...
        window.removeEventListener("message", onMsg);
        try {
          chrome.runtime.sendMessage({ type: "TENCENT_PAGE_FAIL", error: "页面下载看门狗超时（120 秒无进度，CDN 断流或签名过期，请重试）" }).catch(() => {});
        } catch (e) {}
      }, 120000);
      const kick = () => {
        if (closed) return;
      ...
      const onMsg = (ev) => {
        ...
        if (d.__vdm_bili_dash_track_part__) {
          try {
            chrome.runtime.sendMessage({
              type: "TENCENT_PAGE_SEG",
              index: d.index,
              total: d.total,
              b64: abToB64(d.buffer)
            }).catch(() => {});
          } catch (e) {}
          return;
        }
        if (d.__vdm_bili_dash_track_prog__) {
          try {
            chrome.runtime.sendMessage({ type: "TENCENT_PAGE_PROG", done: d.done, total: d.total, bytes: d.bytes || 0 }).catch(() => {});
          } catch (e) {}
          return;
        }
        if (d.__vdm_bili_dash_track_done__) {
          ...
          window.removeEventListener("message", onMsg);
          try {
            if (!d.ok) chrome.runtime.sendMessage({ type: "TENCENT_PAGE_FAIL", error: d.error || "页面下载失败" }).catch(() => {});
            else chrome.runtime.sendMessage({ type: "TENCENT_PAGE_DONE", tally: d.tally || null }).catch(() => {});
          } catch (e) {}
        }
      };
      window.addEventListener("message", onMsg);
      window.postMessage({
        ...
        __vdm_id__: id,
        initSpec: null,
        noCreds: true, // 腾讯 CDN 返回 ACAO:*，分片请求须不带 cookie（CORS 规范），与播放器一致
        rangeChain: !!msg.rangeChain, // Range 链式：CDN 对普通请求切流（16KB 断）时逐段续传
        segments: msg.segments || []
      }, "*");

...
(function (global) {
  ...
  async function download(info, onProgress, concurrency) {
    ...
    const MIN_SEG_BYTES = 1024; // 防御：小响应（如 <1KB）多为 CDN 拒绝/空体，按失败处理
      ...
      while (idx < info.segments.length) {
        ...
        for (let attempt = 0; attempt < 2 && !buf; attempt++) {
          ...
          try {
            ...
            lastErr = "fetch 失败：" + ((e && e.message) || e);
            continue;
          }
          if (!r.ok) { lastErr = "HTTP " + r.status; continue; }
          let ab = await r.arrayBuffer();
          if (ab.byteLength < MIN_SEG_BYTES) { lastErr = "响应过小(" + ab.byteLength + "B)"; continue; }
          if (cryptoKey) {
            const iv = fixedIv || seqToIv((info.mediaSequence || 0) + i);
            try {
              ab = await crypto.subtle.decrypt({ name: "AES-CBC", iv: iv }, cryptoKey, ab);
            } catch (e) {
              lastErr = "解密失败：" + ((e && e.message) || e);
              continue;
            }
          }
          buf = ab;
        ...
        if (!buf) {
          // 连续 5 片失败 → 该 CDN 已拒绝，全部中止
          consecFail++;
          if (consecFail >= 5) {
            throw new Error("连续 " + consecFail + " 段失败（最近：" + (lastErr || "未知") + "），疑似 CDN 拒绝，停止");
          }
          out[i] = null;
        } else {
          consecFail = 0;
    ...
    // 任何 null（失败）槽位都拒绝拼接，避免产出残片
    for (let i = 0; i < out.length; i++) {
      if (!out[i]) throw new Error("分段 " + (i + 1) + " 下载失败（" + info.segments.length + " 片未全部成功），拒绝拼接");
      parts.push(out[i]);
    }

...
(function (global) {
  ...
  async function resolveBestVariant(manifestUrl) {
    ...
    if (/#EXT-X-STREAM-INF/.test(text)) {
      ...
      base = new URL(best.uri, base).href;
      height = best.h;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (L.startsWith("#")) {
        if (L.startsWith("#EXT-X-MAP")) {
          const m = /URI="([^"]+)"/.exec(L);
          if (m) initUrl = new URL(m[1], base).href;
        }
        continue;
      }
      segs.push(new URL(L, base).href);
    }
    if (!segs.length) throw new Error("m3u8 中未找到媒体分段");
    return { height: height, initUrl: initUrl, segments: segs };
  }
